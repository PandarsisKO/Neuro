...
*Cost note (measured while building R2):* "$0" was wrong — `library.recall` embeds its query, so a scan costs one
embedding per search (a fraction of a cent, and the same call every search already makes). **No generation call**
is the invariant worth gating, and it is gated.
...
results. *Cost:* a few query embeddings; no generation call.
...
- **Migration:** additive only. One new table (R2), one new column + one small table (R6). The legacy
  ...
  `legacy_unverified`.
- **Cost:** R2 spends one query embedding per search and nothing else — the "$0" in the original plan was wrong,
  and the gate is *no generation call*, which is stronger and testable. The one place a generation call may later
  be justified is goal expansion on a very short goal (today that case is reported to the user instead), and it
  would be one call cached against the brief revision.
