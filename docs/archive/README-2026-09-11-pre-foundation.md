...
## Historical cloud packaging (unsupported)

Neuro Search's current product contract is single-user and local. The `Dockerfile` and
`fly.toml` remain only as historical packaging evidence while the Product Scheduler parks
cloud architecture and multi-user access. Do not deploy the current build to a public host:
it has one shared token and no project-scoped authorization boundary.

The commands below describe the former experiment and are not a supported installation path.
