...
## T2 implementation and cohort release — 2026-09-13 14:35 PT

The admitted view is implemented at `280146a` through `t1.coverage_view` and the existing authenticated `/api/projects/{project_id}/transcript/coverage` route (`detail=chunks`, bounded paging). It adds no table, provider call, UI, or live-data mutation. The view preserves independent signals, primary-state precedence, typed locator reconciliation, project-relative T1 representation thresholds, and explicit unknown reasons.

The copied-backup cohort `evals/t2/cohort-20260913-1435.json` exercised all three retained projects read-only. It measured 38,038 valid chunk vectors: Design 6,139 chunks in 2.87 s, Buying Businesses 24,166 in 44.99 s, and Real Estate 2,719 in 0.18 s. P90 representation cut points were Claims/Findings 0.7193/0.7084, 0.7170/0.7027, and 0.6971/0.6685 respectively. No malformed finding citations occurred. The 1,091 unmatched Buying Businesses and 37 unmatched Design Claim locator rows remain typed boundary/member/gap evidence.

Focused T2/T1 tests pass 31; the full suite passes 1,368 tests with one existing Starlette deprecation warning. The commit-bound release gate passes at `evals/release/release-check-0.63.65-280146a-20260913-143316.json`. T2's required read-only view and measurement gates are closed; `extracted`, `redundant`, `irrelevant`, and therefore `unexplained` remain explicitly unavailable until their own valid chunk-level inputs exist.
