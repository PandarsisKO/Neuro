...
| Embeddings | OpenAI `text-embedding-3-small` | **TEST — the strongest local candidate** | This is where the hardware pays. A query is ~20 tokens; a local embedding model (a few hundred MB, entirely GPU-resident) answers in single-digit milliseconds against a measured **0.5 s** network round trip — a 25–100× cut on retrieval's largest single cost, plus one fewer network dependency and one fewer paid provider. The one-time re-embed of 26,958 chunks (~13 M tokens) is roughly an hour of idle GPU, free, and belongs in the `low` lane overnight. Real costs: dimension change (1536 → 768 for most open models) rewrites every stored blob, and the frozen retrieval baseline (MRR 0.9093, recall@10 ≥ 0.9) must be re-validated, not assumed. Cache query embeddings first regardless — cheaper, risk-free, and it survives whichever way R9 lands. |
