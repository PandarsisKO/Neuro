> P0 RESPONSIVENESS SUPERSESSION (2026-09-17, Kyle): the ladder below is temporarily narrowed to one gate — foreground user actions stay responsive regardless of background research, maintenance, polling, queue size or open tabs. `docs/SPEED-AUDIT-2026-09-17.md` holds the measured cause (harvest holding the writer for minutes, a 186 s caption-recovery poll, a 92 s `/api/sources` poll, unbounded overlapping polls filling the browser's connections, `*.html` restarts) and the execution record: 0.63.92 shipped P0.1 poll containment, P0.2 harvest isolation, P0.3 incremental caption recovery, P0.4 the `/api/sources` diet + `db:write_hold` ledger, P0.5 Python-only reload. R2/R3 below are not wrong; they regressed by scale (19k notes / 17k Claims / 1.2 GB) and by endpoints added after them. The remaining ladder resumes from fresh measurements once the "queue active" half of the P0 gate is read (§7 of the audit); nothing in it is silently rewritten here.
>
