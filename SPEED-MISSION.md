...
| Local model execution | Ollama · llama.cpp | **TEST (narrowly scoped)** | Both MIT, both healthy. Justified for embeddings and cheap deterministic classification/routing. **Not** for findings/claims extraction — swapping Sonnet-grade comprehension for a 7B local model is precisely the "shallower" trade Principle 13 forbids. Benchmark on the actual Mac; I could not measure that hardware from the sandbox. |
...
| Embeddings | OpenAI `text-embedding-3-small` | **TEST alternative** | The 0.5 s round trip is retrieval's largest cost. A local embedding model removes it and the network dependency. Cost of switching is real: re-embedding 26,958 chunks and re-validating the frozen retrieval baseline (MRR 0.9093, recall@10 ≥ 0.9). Cache query embeddings first — cheaper and risk-free. |
...
**R8 · Storage hygiene and retention.** *Changes:* `cache_size`, `mmap_size`, `temp_store=MEMORY`; scheduled `ANALYZE`; WAL checkpoint policy; indexes on `project_notes(project_id, status, source_id)`, `messages(conversation_id)`, `usage(kind, source_id)`, `jobs(kind, status)`; retention for `job_events`/`invocations`/`usage`; reaper for the 432 stuck `in_flight` rows. *Why:* cheap insurance that keeps §A's numbers true at 10× scale; the `in_flight` leak is a correctness defect today. *Benefit:* small now, decisive later. *Depends:* nothing — may land any time; the reaper should land early. *Risk:* low, additive. *Verify:* `sqlite_stat1` present; no `in_flight` row older than its lease; query plans unchanged or better.

**R9 · Local runtime benchmark (TEST rung, may run in parallel).** *Changes:* benchmark Ollama and llama.cpp on Kyle's actual Mac for embeddings and cheap classification only — model load time, first-token latency, tokens/sec, memory, quality, behaviour under concurrent load. *Why:* decides the embeddings question (0.5 s per search) and whether a genuinely local lane can de-risk the Claude Code dependency. *Benefit:* unknown until measured — that is the point of the rung. *Depends:* R0. *Risk:* scope creep into reasoning tasks; Principle 13 forbids it. *Verify:* a written scorecard; adopt only if it beats the current path on a measured axis without touching findings/claims quality. If it does not, record the negative result and close the question.
