> P0 RESPONSIVENESS — CLOSED 2026-09-17 (Kyle's narrowing of this mission; `docs/SPEED-AUDIT-2026-09-17.md` §7–8). 0.63.92 + `2e386b7` + `0668899`: poll containment, harvest isolation, incremental caption recovery, the `/api/sources` diet, Python-only reload, the `db:write_hold` ledger, and the two writer holds the ledger then exposed under load (`dedupe_targets` 12 s, `synthesize` 3.5 s). Under a continuous $0 research pass: New Chat p50 80 ms / p90 0.63 s, writer peak 0.19 s, zero `database is locked`, zero false lease loss, one `/api/sources` in flight per tab. The invariant is now standing rule in `CLAUDE.md`: *background throughput may consume spare capacity; it may never consume foreground capacity.* **Update 19:20 PT:** R8 (narrow) shipped (`0080d1f`, project job list 210 → 31 ms) and the `/api/sources` cache-churn rung is closed (`3199711` `c3374fd` `fbdd4d5` `42268c6` `9980697`): five writers moved the research revision without changing the answer, plus a cache sized under its working set; under a running research pass the endpoint went from p50 5.2 s to 0.70 s with 98 % hits — audit §9. **Correction 19:40 PT:** R4 and R5 were already shipped (0.63.40/41, see HARDENING.md) — the re-ordered ladder above mis-carried them as future work; R8 is reconciled item by item in audit §10 (mmap and retention open by decision, with triggers); the shipped R4 key was audited against the project-relative identity rule and gated in Kyle's words (test_s46). Remaining on this ladder: R9(c). **Observation mode (Kyle, 2026-09-17 evening, audit §11):** twin-project unit reuse stays as shipped (compute reuse, never project-state reuse — gated); R9(c) stays deferred; process separation reopens only on real-world evidence — foreground p90 > 1 s repeatedly, with write holds < 250 ms, caches > 95 % hit, and traces attributing the delay to CPU-bound background work. No active speed development until then.
>
> **The ladder below is re-ordered by Kyle on that evidence (2026-09-17), and resumes from fresh measurements, not from the 09-09 numbers:** (1) **R8** correctness/hygiene — the cheap indexes (`jobs.project_id`, `api_project_jobs` at 210 ms today), the stuck-row reaper and the retention window, WAL/ANALYZE — low risk, and the 09-11 State of the App flagged a real correctness bug in it; (2) the **remaining request-path diet** — `/api/sources` is 1.69 MB idle and 5–27 s under a running research pass because every research cache keys on a fingerprint that `knowledge.refresh` moves on every pass (0/11,132 `potential` hits measured); measure which bytes and which keys before cutting, and use `/api/perf` to find the next heavy refresh endpoint rather than assuming Sources; (3) **R4** durable partial work — retries stop redoing completed windows (elapsed time and spend, no new concurrency); (4) **R5** in-job concurrency — only after foreground/background isolation is proven at the process level, because concurrency can recreate exactly the contention P0 removed; R9 local-model benchmarking below all of these. R6/R7 entries below are historical (shipped 0.63.42/43). Nothing below is rewritten; this note is the supersession record.
>
>
> FOUNDATION COMPLETE: R4 and its 66-case recovery abuse gate are complete in 0.63.40; R5 and its concurrency gate are complete in 0.63.41; R6 and its Fast/Warm gate are complete in 0.63.42; R7 and its novelty/residual gate are complete in 0.63.43. The authenticated closeout observed an empty current queue, healthy integrity/backup state, and interactive endpoint p90 at or below 0.58s. Transcript Intelligence is now the active program and is governed by DEVELOPMENT-OPERATING-SYSTEM.md and PRODUCT-SCHEDULER.md. R8 immediate evidence is complete (30-day retention sampling continues); R9(a) selected `bge-m3` (Ollama 0.33.3, 1024d) after it cleared frozen latency, recall@10 and MRR gates in an isolated fixture; `nomic-embed-text` was rejected on MRR. R9(b) is complete with Llama 3.1 8B adopted for short classification and the other declared candidates rejected on the measured gate. R9(c) is explicitly deferred with a numeric revisit trigger. Existing production chunk embeddings are unchanged until the versioned migration/backfill completes. R6 and R7 share Foundation Phase 9 but remain separate measured releases. No parallel product missions.
>
...
**R8 · Storage hygiene and retention — IMMEDIATE GATE COMPLETE IN 0.63.36.** *Shipped changes:* bounded per-connection `cache_size`, 1 GB `mmap_size`, `temp_store=MEMORY`; weekly housekeeping-owned `ANALYZE`; the four measured indexes below. Existing WAL checkpoint and terminal/orphan invocation cleanup remain the owners; no duplicate reaper was added. On a copied verified 630 MB backup, source-specific Findings changed from a 7.87 ms scan to a 0.43 ms indexed lookup; message and usage scans were removed; the broad Findings list correctly remained a scan. *Immediate verification:* copied verified backups at 16:28 and 17:28 PT passed integrity and contained `sqlite_stat1`; no growth was measurable across that hour. *Retention decision:* do not delete `usage` or `invocations`, which are authoritative cost/execution evidence. `job_events` remains under a non-destructive 30-day observation until after 2026-10-11 17:28 PT; identify every consumer before admitting a rollup/archive cutoff. Connection memory and query-plan observation remain non-blocking monitoring, not an unclosed R8 implementation gate. Exact evidence is in HARDENING.md and the pre-T1 audit.
...
*(c) Findings/claims extraction — expected to fail, and worth failing on the record.* Run one real 15k-token window through a 70B Q4 and an MoE (gpt-oss-120b), and report prefill time separately from generation time. **Adopt only if** total wall-clock beats 10.9 s (current API parity) and the requested findings/claims JSON shape is valid; I predict 45–90 s, dominated by prefill. Either way the number gets written down and the question closes.
