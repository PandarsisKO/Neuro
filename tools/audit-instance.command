...
if [ ! -f data-audit/neurosearch.db ]; then
  ...
  echo "$(basename "$SRC")  copied $(date '+%Y-%m-%d %H:%M')  baseline $(git rev-parse --short HEAD)" > data-audit/PROVENANCE.txt
else
...
echo "Baseline commit: $(git rev-parse --short HEAD)   data: $HERE/data-audit   fake AI, \$0 budgets"
