...
**Added 2026-09-14, on implementation.** Two more patterns surfaced on a closer pass through the folder, both checked against the live app before being written down here:

Vyra's health-intelligence dashboard tints its vital-sign rows with a small up/down arrow badge (`↗ Normal` in green, `↘ Low` in amber) instead of a bare number. Neuro's `--ok-soft`/`--warn-soft`/`--bad-soft` tokens existed for this (the "soft-tinted status pills" line above) but were used in exactly one place in the whole stylesheet — the intent was documented, not built. Implemented as `.pill-stat` in `styles.css` plus `pillHtml()` in `sources.js`, applied to the three percentage-based rows in the health panel (finding quote validity, finding citation rate, answer citation validity). Deliberately not applied to every `.status-ok/warn/bad` span — most of those are plain inline text color, not pills, and forcing backgrounds onto all of them would restyle dozens of unrelated call sites this pass never touched.

Salach.ai's meetings list shows a specific processing-status subtitle ("AI summary ready · 5 scope points, 4 timeline items") rather than a generic spinner. Checked this against Sources, Chat, and Master Plan build progress: the app already does this consistently — Sources rows read counts and named states, not "loading"; Chat's thinking indicator carries a message; the remaining bare "Loading…" strings (`chats.js`, `research.js`, `sources.js`, `utils.js`) are all legitimate transient initial-page-load states, not violations of the principle. No code changed for this one — recorded here so the next audit doesn't re-open it as a gap.

**Tried 2026-09-14, second follow-up, then reverted — kept as a note against repeating it.** OrchestrateIQ's "Agents" table pairs a percentage with a small inline progress bar in the same cell. Sources' "substance NN/100" tag looked like the same shape, so a `.tag-bar` (`styles.css`, a 28px `currentColor` track) was added inside it in `sources.js`. Kyle caught the mismatch immediately: a progress bar reads as *how much of something is done*, and substance is a static confidence/strength score, not a completion metric — a low-substance source would visually read as "not finished yet" rather than "not much here." Reverted both files. The lesson generalizes: before reusing an inline-bar pattern, check whether the number it is attached to is actually a fill-toward-100 quantity (a rate, a percentage of a whole) or a score/rating on a bounded scale — visually they look the same, but a bar only reads correctly as the former.

Salach.ai's meeting-detail screen also has a category-breakdown block: a colored horizontal bar-and-dot row per objection category (Price/Budget, Fit/Relevance, Authority, Commitment), each category its own color. Checked this against the Reject line below before going further — its entire visual identity *is* "a different tinted colour per category," the thing this document already rejects deliberately (§10: one accent color, not a rainbow of category colors). No de-colorized variant was pursued either: the only candidate landing spot found was the Evidence tab's group breakdown (`plan.js`, U/F/S/C groups), and those groups aren't a natural fit for a bar chart — they're link-out chip lists of arbitrary size, not a bounded percentage split. Skipped; recorded here so a future audit sees this was checked, not missed.
