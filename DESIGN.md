...
**Added 2026-09-14, second follow-up.** OrchestrateIQ's "Agents" table pairs a percentage with a small inline progress bar in the same cell, rather than the bare number alone. Neuro's Sources rows already show a "substance NN/100" tag on every source with a summary — the same shape. Added `.tag-bar` in `styles.css` (a 28px track using `currentColor`, so it automatically inherits whichever status color the parent `.tag.status-ok/warn/bad` span is already set to) and embedded it inside that tag in `sources.js`. Not applied to the `pool_potential` tag on skipped sources — lower visibility, and one bar per row read is enough; adding a second would clutter rather than clarify.
