...
CHAT_ARM_INPUT_TOTAL = 178428   # frozen: plain + cache read + cache write of the 34-question chat arm (re-frozen 0.24.1: library tools + inventory in the state block)
...
def test_router_equivalence_fake_tier1(isolated_db, monkeypatch):
    ...
    assert v["answer"]["calls"] == 34 and tot(v["answer"]) == 165365 and v["findings"]["calls"] == 9 and tot(v["findings"]) == 30297
    assert v["plan"]["calls"] == 2 and tot(v["plan"]) == 11026 and v["plan"]["cache_read"] == 4412
    assert sum(tot(t) for t in v.values()) == 231584
