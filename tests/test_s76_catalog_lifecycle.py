...
def test_run_and_job_admission_rollback_together(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="crash after enqueue"):
        reservoir.request_subreddit_scan(pid, cid, action="attach")
...
def test_refresh_of_blocked_retry_cannot_dedupe_to_the_old_job():
    ...
    refreshed = reservoir.request_subreddit_scan(pid, cid, action="refresh")
    ...
    assert reservoir.request_subreddit_scan(pid, cid, action="refresh") == refreshed
...
def test_cancel_resume_keeps_cursor_and_run_limits(monkeypatch):
    ...
    assert turn() == "queued"
    first = state(pid, cid)
    reservoir.request_subreddit_scan(pid, cid, action="cancel")
    assert db.get_job(jid)["status"] == "cancelled"
    assert reservoir.subreddit_catalogs(pid)[0]["scan"]["status"] == "cancelled"
    monkeypatch.setattr(reservoir, "SUBREDDIT_MAX_PAGES", 50)
    resumed = reservoir.request_subreddit_scan(pid, cid, action="resume")
...
def test_official_http_failure_survives_adapter_and_job(monkeypatch, status):
    ...
    assert saved["http_status"] == status and saved["status"] == "blocked"
    ...
    if expected == "queued":
        ...
        assert reservoir.request_subreddit_scan(pid, cid, action="resume")["job_id"] == jid
...
def test_retry_after_http_date_and_invalid_header():
    ...
    assert community._api_refusal(429, {"retry-after": formatdate(future, usegmt=True)}).retry_at == future
    assert community._api_refusal(503, {"Retry-After": "nonsense"}).retry_at is None
...
def test_retry_attempts_are_bounded_and_success_clears_old_error(monkeypatch):
    ...
    def fail(*_a, **_kw):
        raise community.RedditAPIError("busy", status=503, kind="server", retryable=True)
    ...
    assert turn() == "done"
    assert "error" not in state(pid, cid) and "http_status" not in state(pid, cid)
...
def test_missing_credentials_and_untyped_errors_do_not_retry_by_message(monkeypatch):
    ...
    reservoir.request_subreddit_scan(pid, cid, action="resume")
    def fail(*_a, **_kw):
...
def test_new_scan_actions_reject_foreign_projects_before_writes():
    ...
    for action in ("attach", "refresh", "resume", "cancel"):
        ...
        with pytest.raises(ValueError):
            reservoir.request_subreddit_scan(foreign, cid, action=action)
...
def test_concurrent_double_click_admits_one_run_and_job():
    ...
    def admit():
        try:
            gate.wait(timeout=5)
            return reservoir.request_subreddit_scan(pid, cid, action="refresh")
        finally:
            db.close_thread_connection()
...
def test_legacy_checkpoint_resumes_with_new_job_binding(monkeypatch):
    ...
    saved = reservoir.request_subreddit_scan(pid, cid, action="resume")
