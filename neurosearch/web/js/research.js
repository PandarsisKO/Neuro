...
globalThis.renderTriageCard = async function renderTriageCard(elId) {
  ...
  const rebuildBtns = (x, key) => `<button class="small ${key === 'rebuild_transcript' && !x.local_line ? 'primary' : ''}" title="${t.local ? 'Runs on Claude Code in the background — $0, but slow' : 'Runs on the API'}" onclick="rebuildTier('${key}')">Rebuild · ${esc(x.cost_line)}</button>${fastBtn(x, key, 'Rebuild')}`;
