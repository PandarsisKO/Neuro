...
globalThis.srcRowHtml = function srcRowHtml(s) {
  ...
  return `<div class="src ${(s.job && s.job.status === 'running') || s.analysing ? 'live' : ''}">
    ...
    <div class="grow min-w-0">
      ...
      ${s.summary ? `<div class="muted mt-1">${s.legacy_analysis ? `<span class="tag status-warn" title="Preserved from Neuro Search 0.15 — its original project context cannot be verified. Re-analyse to replace it.">⚠ legacy analysis</span> ` : ''}${s.substance != null ? `<span class="tag ${s.substance >= 60 ? 'status-ok' : s.substance >= 30 ? 'status-warn' : 'status-bad'}">substance ${s.substance}/100<span class="tag-bar" style="--pct:${s.substance}%"></span></span> ` : ''}${esc(s.summary)}</div>` : ''}
