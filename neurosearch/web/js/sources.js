...
globalThis.loadReviews = async function loadReviews(quiet) {
  ...
  $('#reviewWrap').innerHTML = rvs.map((c, idx) => {
    ...
    // S89 (Kyle): "I had no idea that the Sharran card was folded... thats not obvious enough. I was honestly
    // perplexed." A folded card kept its header, rank line and select-all/none/filter row, so it read as an EMPTY
    // list. Folded now means: a "list folded" tag by the title, no list controls, and one primary button that says
    // what it will show — the fold itself (R8: only the first card open, so the queue stays reachable) is unchanged.
    ...
    return `<div class="card rv${folded ? ' folded' : ''}" style="border-color:var(--warn)" id="rv-${c.id}">
      <div class="row"><b class="grow">Review before starting: ${esc(c.title || c.kind)}<span class="rvfold" onclick="RVFOLD['${c.id}']=!(RVFOLD['${c.id}'] !== undefined ? RVFOLD['${c.id}'] : ${idx > 0});globalThis.rvSig =null;loadReviews()">${folded ? '▸ show' : '▾ hide'}</span>${folded ? `<span class="tag status-warn" style="margin-left:8px;font-weight:normal">list folded</span>` : ''}</b><span class="muted"><span id="rvsel-${c.id}">${nSel}</span> of ${n} selected${mins ? ` · ${Math.round(mins / 6) / 10} h listed` : ''}</span></div>
      ...
      ${folded ? `<div class="row" style="margin-top:8px"><button class="small primary" onclick="RVFOLD['${c.id}']=false;globalThis.rvSig=null;loadReviews()">▾ Show the ${n} videos to review</button><span class="muted">Folded so the cards below stay reachable — nothing here is selected or started until you open it and press Start.</span></div>` : ''}
      <div class="row rvctl" style="margin-top:6px"><button class="small ghost" onclick="rvAll('${c.id}', true)">select all</button><button class="small ghost" onclick="rvAll('${c.id}', false)">none</button><input placeholder="filter titles…" style="max-width:240px" value="${esc(rvFilterText[c.id] || '')}" oninput="rvFilter('${c.id}', this.value)"></div>
...
globalThis.CATALOG = { id: null, mode: 'recommended', state: 'available', page: 0, revision: null };
